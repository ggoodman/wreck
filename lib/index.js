...
const internals = {
    ...
    shallowOptions: ['agent', 'agents', 'beforeRedirect', 'downstreamRes', 'client', 'payload', 'redirected']
};
...
internals.Client.prototype.request = function (method, url, options = {}) {
    ...
    try {
        ...
        Hoek.assert(options.client === undefined || !(typeof options.client === 'object') || typeof options.client.request !== 'function', 'options.client must be a an object having a request property that is a function');
    }
    catch (err) {
...
internals.Client.prototype._request = function (method, url, options, relay, _trace) {
    ...
    const client = options.client || (uri.protocol === 'https:' ? Https : Http);
